...
const PopularDestinations = () => (
  <section className="max-w-[1200px] mx-auto px-4 mt-14">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
      {popularDestinationsData.filter(item => item.size === "large").map((item, idx) => (
        <div key={idx} className="relative rounded-lg overflow-hidden cursor-pointer shadow-md hover:shadow-lg transition">
          <img
    ...
    <div className="grid grid-cols-3 sm:grid-cols-5 gap-6">
      {popularDestinationsData.filter(item => item.size === "small").map((item, idx) => (
        <div
          key={idx}
          className={`relative rounded-lg overflow-hidden cursor-pointer shadow-sm hover:shadow-md transition ${item.hiddenOnMobile ? 'hidden sm:block' : ''}`}
        >
          <img
            alt={`${item.title} scenic view`}
